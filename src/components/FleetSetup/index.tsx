...
class FleetSetupFounder extends React.PureComponent<IFleetSetupFounderProps> {
    @autobind
    addOwnerShip() {
        this.props.addOwnershipItemComponent(this.props.ownerShipItemComponents.length + 1);
    }
    removeOwnerShip(index) {
        this.props.removeOwnershipItemComponent(index);
    }
    handleSubmit = e => {
        e.preventDefault();
        this.props.form.validateFields((err, values) => {
            if (!err) {
                let ownerships = Object.keys(values).filter((k) => { return k.startsWith('ownership-name'); }).map((k, index) => {
                    let elementIndex = k.substr('ownership-name'.length);
                    return { name: values[k], number: values['ownership-number' + elementIndex] };
                });
                this.props.setup({ shippingLine: values.shippingLine, name: values.name, ownerships: ownerships });
            }
        });
    };
    public render() {
        const { getFieldDecorator, getFieldsError, getFieldError, isFieldTouched } = this.props.form;
        var ownerShipItemComponents = this.props.ownerShipItemComponents.map((item, index) => {
            return <Row gutter={16} key={item.name + index}>
                <Col span={6}>
                    <Form.Item>
                        {getFieldDecorator('ownership-name' + index, {
                            rules: [{ required: true, message: 'Please input name!' }],
                        })(
                            <Input prefix={<FleetSetupIcon style={{ color: 'rgba(0,0,0,.25)' }} />} placeholder="Name" />
                        )}
                    </Form.Item>
                </Col>
                <Col span={6}>
                    <Form.Item >
                        {getFieldDecorator('ownership-number' + index, {
                            rules: [{ required: true, message: 'Please input number!' }],
                        })(
                            <Input prefix={<FleetSetupIcon style={{ color: 'rgba(0,0,0,.25)' }} />} placeholder="Number" />
                        )}
                    </Form.Item>
                </Col>
                <Col span={6}>
                    <Button shape="circle" icon="sub" onClick={() => { this.removeOwnerShip(index); }} />
                </Col>
            </Row>
        });
        return (
            <Form onSubmit={this.handleSubmit}>
                    ...
                    <Col span={6}>
                        <Form.Item>
                            {getFieldDecorator('shippingLine', {
                                ...
                                <Input prefix={<FleetSetupIcon style={{ color: 'rgba(0,0,0,.25)' }} />} placeholder="Shipping Line" />
                            )}
                        </Form.Item>
                    </Col>
                    <Col span={6}>
                ...
                <Row gutter={16}>
                    <Col span={24}>
                        <Button shape="circle" icon="plus" onClick={this.addOwnerShip} />
                    </Col>
                </Row>
                <Row gutter={16}>
                    <Col span={6}>
...
export default connect()(Form.create({ name: 'fleet_setup' })(FleetSetupFounder));