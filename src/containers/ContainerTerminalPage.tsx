...
import { initContainerTerminalPage, demolishContainerTerminal, constructContainerTerminal, addBerthComponent, removeBerthComponent, initShippingLineDropdownList } from '../actions';
...
export interface IContainerTerminalPageProps {
    ...
    constructContainerTerminal: (any) => void;
...
class ContainerTerminalPage extends React.Component<IContainerTerminalPageProps, any> {
    ...
    render() {
        return (
            <div>
                <Row>
                    <ContainerTerminalConstruction construct={this.props.constructContainerTerminal} reload={this.props.initContainerTerminalPage} berthComponents={this.props.berthComponents}
                ...
                <Row>
                    <List grid={{ gutter: 16, column: 4 }} dataSource={this.props.containerTerminals}
                        renderItem={(item: ContainerTerminal) => (
                            <List.Item>
                                <Card actions={[
                                    <Popconfirm title={<FormattedMessage id="Confirm.Delete" values={{ name: item.name }} />} onConfirm={() => { this.props.demolishContainerTerminal({ shippingLine: item.shippingLine, name: item.name }); }} okText={<FormattedMessage id="Label.OK" />} cancelText={<FormattedMessage id="Label.Cancel" />}><Icon type="delete" /></Popconfirm>,
                                    <Icon type="help" />,
                                    <Icon type="ellipsis" />]}>
